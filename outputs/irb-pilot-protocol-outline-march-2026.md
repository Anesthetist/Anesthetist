---
created: '2026-03-30'
creator: Claude (Vigil + Researcher)
id: urn:srl:output:irb-pilot-protocol-outline-march-2026
modified: '2026-03-30'
prov:wasDerivedFrom:
- urn:srl:observation:measurement-taxonomy-interoception-deep-dive-2026
- urn:srl:observation:clinical-measurement-instruments-compendium
- urn:srl:evidence:wana-conference-talk-march-2026
- urn:srl:evidence:goessl-2017-hrv-biofeedback-stress-anxiety
- urn:srl:evidence:remote-hrvb-mental-health-meta-analysis-2025
status: draft
subject:
- IRB
- pilot-study
- HRV-biofeedback
- interoception
- CRNA
- burnout
- clinical-trial
- measurement
title: IRB Pilot Protocol Outline — CRNA HRV Biofeedback + Interoceptive Awareness
  Training (March 2026)
type: output
---

# IRB Pilot Protocol Outline: CRNA HRV Biofeedback + Interoceptive Awareness Training

## Summary

Working draft IRB protocol for a pilot RCT (N=40, 6-week intervention, waitlist control) testing mobile HRV biofeedback + interoceptive awareness training for burnout reduction in CRNAs. Uses the validated CRNA Enterprise Pilot battery (Mini-Z + BRS + ISI + PSS-10 + MAIA-2 subscales; 33 items, ~8 min). Estimated budget $30-37K. Full document in outputs folder.

## Key Design Decisions

- **6-week intervention** based on comparable RCTs (2026 ED/ICU HRVB trial used 4 weeks, d=0.79)
- **Three-component intervention**: daily HRV biofeedback, transition-window micro-practices, weekly interoceptive awareness modules
- **All free instruments**: no licensing costs for the measurement battery
- **MAIA-2 subscales only** (Noticing + Self-Regulation) to keep battery brief while capturing interoceptive change
- **Dual primary focus**: feasibility metrics AND preliminary effect sizes
- **Physiological data from consumer wearables** under FDA Jan 2026 General Wellness Policy
- **No IND/IDE required**

## Strategic Purpose

This pilot unblocks: NIH SBIR Phase I application, AANA CE submission with outcome data, enterprise sales with clinical evidence, and the definitive multi-site RCT. It is Vigil's #1 identified strategic gap.
